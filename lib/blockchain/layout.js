 ...
 *   d[addr-hash][height][hash][index][change] -> dummy (debit by address, height)
 *   B[addr-hash][height][hash][index][change] -> dummy (credit by address, height)
 *   w[height] -> name undo
 *   s -> tree state
 *   f -> bit field
...
const layout = {
  ...
  d: bdb.key('d', ['hash', 'uint32', 'hash256', 'uint32', 'uint64']),
  B: bdb.key('B', ['hash', 'uint32', 'hash256', 'uint32', 'uint64']),
  w: bdb.key('w', ['uint32']),
  s: bdb.key('s'),
  f: bdb.key('f'),
  M: bdb.key('M', ['uint32'])
