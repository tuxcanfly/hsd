...
class ChainDB {
  ...
  async getBalanceAt(addr, height) {
    ...
    const balances = await this.db.values({
      gte: layout.B.min(hash),
      lte: layout.B.max(hash, height),
      parse: (key) => { return new bio.BufferReader(key).readI64(); }
    });
  ...
  indexTX(tx, view, entry, index) {
    ...
    if (!tx.isCoinbase()) {
      for (const {prevout} of tx.inputs) {
        ...
        const balance = new bio.StaticWriter(8).writeI64(-coin.value);
        this.put(layout.B.encode(addr, entry.height), balance.render()); 
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      const balance = new bio.StaticWriter(8).writeI64(output.value);
      this.put(layout.B.encode(addr, entry.height), balance.render()); 
