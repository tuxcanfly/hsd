  ...
  indexTX(tx, view, entry, index) {
    ...
    if (!tx.isCoinbase()) {
      for (const {prevout} of tx.inputs) {
        ...
        this.put(layout.d.encode(addr, entry.height, coin.value), null);
      }
    }
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      // Registers are burned.
      if (output.covenant.isRegister()) {
        this.put(layout.d.encode(addr, entry.height, output.value), null);
        continue
      }
      ...
      this.put(layout.B.encode(addr, entry.height, output.value), null);
    }
  }
  ...
  unindexTX(tx, view) {
    ...
    if (!tx.isCoinbase()) {
      for (const {prevout} of tx.inputs) {
        ...
        this.del(layout.d.encode(addr, entry.height, coin.value));
      }
    }
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      this.del(layout.B.encode(addr, entry.height, output.value));
    }
  }
}
