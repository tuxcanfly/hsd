...
class ChainDB {
  ...
  async getCoinsByAddress(addrs) {
    if (typeof addrs === 'string')
      addrs = Address.fromString(addrs)
  ...
  /**
   * Get balance of address at height.
   * @private
  ...
  async getBalanceAt(addr, height) {
    ...
    const debits = await this.db.keys({
      gte: layout.d.min(hash),
      lte: layout.d.max(hash, height),
      parse: (key) => {
        const [,,,, debit] = layout.d.decode(key);
        return debit;
      }
    });

    const credits = await this.db.keys({
      gte: layout.B.min(hash),
      lte: layout.B.max(hash, height),
      parse: (key) => {
        const [,,,, credit] = layout.B.decode(key);
        return credit;
      }
    });
  ...
  indexTX(tx, view, entry, index) {
    ...
    if (!tx.isCoinbase()) {
      for (const {prevout} of tx.inputs) {
        ...
        if (coin.covenant.type >= types.REGISTER
          && coin.covenant.type <= types.REVOKE) {
          continue;
        }

        this.del(layout.C.encode(addr, hash, index));
        this.put(layout.d.encode(addr, entry.height, hash, index, coin.value), null);
      }
    }
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      // Registers are burned.
      if (output.covenant.isRegister()) {
        this.put(layout.d.encode(addr, entry.height, hash, index, output.value), null);
        continue
      }
      ...
      this.put(layout.B.encode(addr, entry.height, hash, i, output.value), null);
    }
  }
  ...
  unindexTX(tx, view) {
    ...
    if (!tx.isCoinbase()) {
      for (const {prevout} of tx.inputs) {
        ...
        this.del(layout.d.encode(addr, entry.height, hash, index, coin.value));
      }
    }
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      ...
      this.del(layout.B.encode(addr, entry.height, hash, i, output.value));
    }
  }
}
